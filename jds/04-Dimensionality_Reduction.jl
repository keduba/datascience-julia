...
# ╔═╡ 9d6399f2-b321-406d-8b61-a2daa17309ab
begin
	using UMAP
    using Makie
    using XLSX
...
# ╔═╡ 1a487367-2b24-4ab5-afb0-a23cf1545a6f
begin
using StatsBase
using Statistics
using LinearAlgebra
...
# ╔═╡ 5283cde9-5dfc-420a-a7de-dc48d6fdf8ba
using GLMakie

# ╔═╡ 00cb7da6-b0c8-11ef-2a64-09b6da1d374f
md"""

# Dimensionality Reduction

The idea of reducing your feature set to a much smaller number.
- Useful for detecting similar samples.
- Three techniques are covered here, namely:
   1. t-SNE
   2. PCA
   3. UMAP
"""

# ╔═╡ 8b499f32-8339-4e7a-adc4-55ac8e787752
md"The dataset for this task is from `VegaDatasets` package, for specs of over 400 car models."

# ╔═╡ a1e48a42-334a-4cec-9153-189a314e0b47
dc = DataFrame(VegaDatasets.dataset("cars"))

# ╔═╡ a9172d6c-15bd-4079-89a0-c96009aa66e1
begin
	dropmissing!(dc)
	mc = Matrix(dc[!,2:7])
	names(dc)
...
# ╔═╡ d9094b9a-5a42-4c78-9cb5-4127f2192bae
begin
	car_origin = dc[!, :Origin]
	carmap = labelmap(car_origin)
	uniqueids = labelencode(carmap, car_origin)
end

# ╔═╡ 072f07c1-b6e8-4bf7-a795-3e031b697c3f
md"""## 1. PCA

First, we center the data.
"""

# ╔═╡ adea9056-8a8c-4494-9db1-dd24083d60bf
mc

# ╔═╡ 2584690e-17cf-4f59-bfcb-01c9400965e6
data = (mc .- mean(mc, dims=1)) ./ std(mc, dims=1)

# ╔═╡ e91d54ff-96b9-464b-baf4-8d934a5e5caa
# We transpose the data matrix to make each car a column.
data'

# ╔═╡ 72ce5fa6-09b7-4793-9564-9c740095ab37
md"Next, we'll fit the model via `PCA`. We'll also want two output `maxoutdim`."

# ╔═╡ a665adf3-b96f-4f88-8003-69cb7ce3823f
p = fit(PCA, data', maxoutdim=2)

# ╔═╡ 74b462b3-a806-4503-97bf-4399458d46a3
# the projection matrix
pm = projection(p)

# ╔═╡ 340ed62e-0fa3-4de1-9f89-8a9fd3e345c7
fieldnames(typeof(p))

# ╔═╡ 1722a418-5f66-4ace-ad38-f30b6f76af07
p.proj

# ╔═╡ ed98ab88-ed0d-4fc0-bbd2-32b7c3a41e94
p.prinvars

# ╔═╡ 1a33d7d1-3d1a-40c4-a1dc-cf94e10f8e2b
p.tprinvar

# ╔═╡ 2b956f04-f035-42ca-9aab-06e96a9daa48
p.tvar

# ╔═╡ 7651076d-da28-4d05-8f3a-660699950d65
# apply the projection matrix `pm` to one car
pm' * (data[1, :] - p.mean)

# ╔═╡ 744e23e9-dca8-49a5-82c7-75a05d470a96
# apply the projection matrix `pm` to all cars
yte = MultivariateStats.transform(p, data')

# ╔═╡ 9dcca984-8ba3-4efc-a1fa-abda362f1feb
begin
using CairoMakie
mg = Makie.scatter(yte[1,:], yte[2,:])
end

# ╔═╡ a9ad75f1-87bc-468c-ac9a-4e6bdff50300
md" It's also possible to go from 2 dimensions to the original 6 dimensions using the `reconstruct` function. The result is approximate."

# ╔═╡ 5b8a7bee-adfd-48a8-89f7-51e1f76f3bb1
xte = reconstruct(p, yte)

# ╔═╡ 3ffa7bb3-3faa-4c7e-af14-58c55dbc4df1
data'

# ╔═╡ 6bf4d62d-902e-4abc-bc9d-a40325948ab8
norm(xte - data')

# ╔═╡ 20b2098d-5042-4d44-8f9c-45d39d58d9b9
md"A scatter plot of our PCA transformation"

# ╔═╡ fe5c50d5-79b1-4fa6-b203-ea911ea2cfd6
Plots.scatter(yte[1,:], yte[2,:])

# ╔═╡ fe368df2-7a04-4dd8-8e12-16dc34dfaf71
begin
...
# ╔═╡ 0341cb9c-e8ee-4fba-8dd0-d1cbfcf97b56
md"Let's increase them to 3 output dimensions"

# ╔═╡ dcdb02a6-4e0e-4717-a6f8-c00e8cd93a20
p2 = fit(PCA, data', maxoutdim=3)

# ╔═╡ 22a78b27-c382-4b3d-a30c-f49f4a85fa0f
yte2 = MultivariateStats.transform(p2, data')

# ╔═╡ 223617a4-6316-40bb-bdd8-43c4d98ae8dd
scatter3d(yte2[1, :], yte2[2, :], yte2[3, :], color=uniqueids, legend=false)

# ╔═╡ 207ef854-7c3f-4b36-ad5f-1bc1b05df276
scene = Makie.scatter(yte2[1, :], yte2[2, :], yte2[3, :], color=uniqueids,)

# ╔═╡ 49fc99ea-74d4-42dd-9f40-d5d6d54ed951
md"""

### 2. t-SNE
The next method we'll look at is the t-SNE (**t-Distributed Stochastic Neighbour Embedding**). Though there are several ways to use t-SNE from Julia, we'll be using the `ScikitLearn` interface.

"""

# ╔═╡ 87b67948-cf7c-41b7-ae0e-7541c1d54ba9
@sk_import manifold : TSNE


# ╔═╡ ae6a632c-8d88-4329-a058-3580fce2b809
begin
	tfn = TSNE(n_components=2)
    yt2 = tfn.fit_transform(data)
end

# ╔═╡ fe287178-3955-446f-b1e7-ad9bc1dc8f3c
Plots.scatter(yt2[:, 1], yt2[:, 2], color=uniqueids, legend=false,
	size=(400, 300), markersize=3
)

# ╔═╡ 17e60157-9f35-437d-aeab-78f6853933c5
md"""

### 3. UMAP
The last dimensionality method to apply will be `UMAP`.
"""

# ╔═╡ 30a0b172-2e2f-4f3d-bccf-17dd5754ad18
begin
	lmp = cor(data, data, dims=2)
	embedding = umap(lmp, 2)
end

# ╔═╡ aa61be4d-f16e-4f1b-a4fc-420a9b0b6bae
Plots.scatter(embedding[1,:], embedding[2,:], color=uniqueids)

# ╔═╡ 8306aa81-0d86-4e5f-9783-6bffa0aecba4
begin
    lmp2 = pairwise(Euclidean(), data, data, dims=1)
    embedding2 = umap(-lmp2, 2)
end

# ╔═╡ a3eb7320-6c1e-45e0-a6a1-8dc5e31c6fed
Plots.scatter(embedding2[1,:], embedding[2,:], color=uniqueids)

# ╔═╡ c8b014d7-6c8e-4c5a-9fd5-34258783227f
md"""

The European and Japanese cars are similar while the American cars form two clusters based on their own specs.
"""
...
manifest_format = "2.0"
project_hash = "508f5ff7f35535546cdde84b772c95a5c5ef92d2"
