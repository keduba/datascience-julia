...
# ╔═╡ 7f9eea8a-64ae-47b9-a667-bc680c7a76e8
# First the packages
begin
	using Plots
	using Statistics
...
# ╔═╡ 6055557b-6567-4c9b-aa93-db364e9a6ce3
begin
	using XLSX
	using DataFrames
end

# ╔═╡ 0d7dd550-c3ca-11ef-10c8-19cc469457e1
ENV["GKS_ENCODING"] = "utf-8"

# ╔═╡ e269eb10-2411-479e-9183-22a6189e8536


# ╔═╡ d71c1260-6fae-4b8f-815a-82722ecf525f
...
# ╔═╡ 31e16684-6a6c-4e41-acbe-d130e0c5ea53
begin
	xticklabels = ["one", "five", "six", "fourteen"]
    p = plot(rand(15), xticks = ([1, 5, 6,14], xticklabels), xrotation=90, xtickfont=font(13))
end

# ╔═╡ ce223629-934f-4d26-b10b-82433a996b1c


# ╔═╡ a134f1c1-0692-4a77-8f84-1f379025fa3d
function pad_empty_plot(p)
	ep = plot(grid=false, legend=false, axis=false,
	framestyle=:box)
	newplot = plot(p, ep, layout=@layout([a{0.99h}; b{0.001h}]))
	return newplot
end

# ╔═╡ c1ad9c5d-f420-4c15-80c1-558a7cf9bdc3
pad_empty_plot(p)

# ╔═╡ af29f69f-3590-4828-a0cc-598612fb8bbb
md"#### Time to get data for the notebook"

# ╔═╡ f80c4b6f-3380-46fc-8fa3-bfe21254aa32
dd = DataFrame(XLSX.readtable(joinpath("data", "zillow_data_download_april2020.xlsx"), "Sales_median_price_city"))

# ╔═╡ 3aa66e58-d1b2-47a8-a621-0116c4544ae0
DataFrame(dd)

# ╔═╡ 27fbbbfd-786a-4296-b36c-4698d92fbef6
XLSX.readdata("data/zillow_data_download_april2020.xlsx", "Sale_counts_city", "A1:F9")

# ╔═╡ 41ae62db-272d-47c1-af9f-d7a654c246d6
begin
	dropmissing!(dd)
	states = dd[:, :StateName]
end

# ╔═╡ 61b24599-3de3-429f-bb48-a027d4bedbe7
begin
    nyids = findall(states .== "New York")
	nyframe = dropmissing(dd[nyids, :])
	caids = findall(states .== "California")
	caframe = dropmissing(dd[caids, :])
	flids = findall(states .== "Florida")
	flframe = dropmissing(dd[flids, :])
end

# ╔═╡ bb5da9f9-a864-4656-ab1a-b2be21d0b1b3
md"""

#### Plot 1: Symmetric violin plots and annotations
We'll see the distribution of house prices in the data we have.
"""

# ╔═╡ a2c81ed5-ba50-42e1-9e20-52310a117225
begin
...
# ╔═╡ 5d5ca444-2928-4057-9c91-24b1fb23695a
begin
...
# ╔═╡ 43db6e66-067c-4b4f-a8ee-089abbd4ba08
md"In the following plots we shall try to avoid symmetry in the violin plots. Let's see how it will turn out."

# ╔═╡ b13e0034-7a36-439a-bec3-af3def1f4d73
begin
...
# ╔═╡ 4ef9a41f-5855-4bd2-bb1b-397ad49b3855
...
# ╔═╡ deca3dc9-5fbe-435b-a4e7-51fe45a7ea56
begin
...
# ╔═╡ 6fdbf5f3-27b4-473b-8351-8a8bbb329a0e
md"The plots look similar except for an upward shift after 10 years. Now to a more informative plot. We'll use only one color this time."

# ╔═╡ 31c627a9-cb0a-4bd4-856c-501951b3eb01
begin
	violin(["New York"], ny10, alpha=0.8, color=7, side=:left, label="")
	violin!(["California"], ca10, alpha=0.8, color=7, side=:left, label="2010-02")
	violin!(["Florida"], fl10, alpha=0.8, color=7, side=:left, label="")
	violin!(["New York"], ny, alpha=0.8, color=6, label="2020-02", side=:right)
	violin!(["California"], ca, alpha=0.8, color=6, side=:right, label="")
	violin!(["Florida"], fl, alpha=0.8, color=6, side=:right, label="")
end

# ╔═╡ 8f9895a8-dd4c-4eb5-8d3e-ee7e1f5f4288
begin
    ca19 = caframe[!,Symbol("2019-02")]
	ny19 = nyframe[!,Symbol("2019-02")]
	fl19 = flframe[!,Symbol("2019-02")]
end

# ╔═╡ 7163a209-2421-49b1-bac1-8952ebf0fdcc
caframe

# ╔═╡ 3c845d71-91a7-4b38-8bd2-feafcf9e7cc3
begin
	violin(["New York"], ny19, alpha=0.8, color=7, side=:left, label="")
	violin!(["California"], ca19, alpha=0.8, color=7, side=:left, label="2019-02")
	violin!(["Florida"], fl19, alpha=0.8, color=7, side=:left, label="")
	violin!(["New York"], ny, alpha=0.8, color=6, label="2020-02", side=:right)
	violin!(["California"], ca, alpha=0.8, color=6, side=:right, label="")
	violin!(["Florida"], fl, alpha=0.8, color=6, side=:right, label="")
end

# ╔═╡ e69f749e-4c2f-486f-8aa7-821b90d45bfb
begin
	ep = 0.1
    for (i, ct) in zip((0.0, 1.0, 2.0), (ny, ca, fl))
	    m = median(ct)
		annotate!([(i+0.5+ep, m+0.05, text(m/1000, 10, :left))])
	end
	plot!(xtickfont=font(10), size=(500, 300))
end


# ╔═╡ dd34dff4-1e17-42b3-a31d-3649de1b2e5c
md"Now we'll include more states and annotate both sides"

# ╔═╡ 96705279-7d19-4c0e-bb04-d18ee5e7cb2d
Symbol.("2020-10","2020-02")

# ╔═╡ 5e442257-29fb-43f0-aa5f-09a19aa4292a
...
manifest_format = "2.0"
project_hash = "4bbd3e6417de679f87415ad08edfdac1bd1b8e8e"
